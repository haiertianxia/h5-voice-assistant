export class AudioRecorder {
  ...
  cancel() {
    ...
    this._cleanup(); this.isRecording = false;
    this._stopWaveform(); clearTimeout(this.silenceTimer);
  }

  _cleanup() {
    if (this.audioContext) { this.audioContext.close(); this.audioContext = null; }
    if (this.stream) { this.stream.getTracks().forEach(t => t.stop()); this.stream = null; }
    this.analyser = null; this.mediaRecorder = null;
  }
  ...
  _checkAudioLevel() {
    ...
    let sum = 0;
    for (let i = 0; i < buf.length; i++) { const v = Math.abs(buf[i] - 128) / 128; sum += v * v; }
